	...
	<div class="content">
		...
		<div class="container-fluid">
			...
			<div class="row">
				<div class="col-8">
					<div class="card-box">
						<form action="<?= site_url('master/aset/update') ?>" method="POST" class="needs-validation" novalidate>
							<div class="form-group">
								<label for="aset_code">Kode Aset</label>
								<input type="text" name="aset_code" value="<?= $aset['aset_code'] ?>" class="form-control" readonly>
							</div>
							<div class="form-group">
								<label for="aset_name">Nama Aset</label>
								<input type="text" name="aset_name" value="<?= $aset['aset_name'] ?>" class="form-control" required>
							</div>
							<div class="form-group">
								<label for="aset_unit">Satuan Aset</label>
								<input type="text" name="aset_unit" value="<?= $aset['aset_unit'] ?>" class="form-control" required>
							</div>
							<div class="form-group">
										...
										<optgroup label="<?= $r1['head_code'] . ' ' . $r1['head_name'] ?>">
											<?php foreach ($sub as $r2) : ?>
												<?php if ($r2['head_code'] == $r1['head_code']) : ?>
													<option <?= $aset['subhead_code'] == $r2['subhead_code'] ? 'selected' : '' ?> value="<?= $r2['subhead_code'] ?>"><?= $r2['subhead_code'] . ' ' . $r2['subhead_name'] ?></option>
												<?php endif ?>
											<?php endforeach ?>
										</optgroup>
									<?php endforeach ?>
								...
								<select class="form-control select2" name="account_no" required>
									...
									<optgroup label="Aktiva Tetap">
										<?php foreach ($coa as $c) : ?>
											<option <?= $aset['account_no'] == $c['account_no'] ? 'selected' : '' ?> value="<?= $c['account_no'] ?>"><?= $c['account_no'] . ' ' . $c['account_name'] ?></option>
										<?php endforeach ?>
									</optgroup>
